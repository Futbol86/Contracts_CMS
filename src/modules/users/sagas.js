...
import {UPDATE_PROFILE_ACTION, UPDATE_PF_PASSWORD_ACTION, LOAD_PROFILE_ACTION, UPLOAD_DEALER_LOGO, ADD_A_USER_LOG_ACTION} from './actions';
import {
...
export function* doUpdateProfile({ payload }) {
    try {
        ...
        if(data.data && data.data.status !== 1) {
            yield put({ type: PROFILE_FORM_NAME_FAILURE, payload: {_error: data.data.msg} });
        } else {
...
export function* doUpdatePassword({ payload }) {
    try {
        ...
        if(data && data.data.status === 0) {
            yield put({ type: PROFILE_PASSWORD_FORM_NAME_FAILURE, payload: { _error: data.data.msg } });
            return;
        }
        yield put({ type: PROFILE_PASSWORD_FORM_NAME_SUCCESS, payload: data });
    }
    catch (error) {
        const errors = Object.assign({}, error.errors, { _error: 'Lỗi xảy ra. Vui lòng kiểm tra lại thông tin!' });
...
function* doAddAUserLog({ payload }) {
    try {
        yield put({ type: ADD_A_USER_LOG_ACTION.LOADING });
        const data = yield call(api.apiAddAUserLog, payload);
        yield put({ type: ADD_A_USER_LOG_ACTION.SUCCESS, payload: data });
    }
    catch (error) {
        yield put({ type: ADD_A_USER_LOG_ACTION.FAILURE, payload: {errors: error} });
    }
}
...
[
    ...
    takeLatest(ADD_A_USER_LOG_ACTION.ACTION,            doAddAUserLog)
];