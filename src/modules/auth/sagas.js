...
export function* doLogin({ payload }) {
    try {
        ...
        //     yield put({ type: LOGIN_FORM_NAME_FAILURE, payload: { _error: '"Tài khoản" hoặc "Mật khẩu" không chính xác!' } });
        // }
        if(data.data && data.data.status === 0) {
            yield put({ type: LOGIN_FORM_NAME_FAILURE, payload: { _error: data.data.msg } });
        } else {
        ...
        const errors = Object.assign({}, error.errors, { _error: 'Đăng nhập thất bại. Vui lòng kiểm tra lại!' });
        //-- The following dispatch will also call SubmissionError
        yield put({ type: LOGIN_FORM_NAME_FAILURE, payload: errors });
    }
}

export function* doJWTLogin() {
    try {
        yield put({ type: LOGIN_ACTION.LOADING });
...
export function* doForgotRequest({ payload }) {
    try {
        yield put({ type: FORGOT_REQUEST_ACTION.LOADING });
        const data = yield call(api.apiDoForgotRequest, payload);

        if(data.data && data.data.status === 0) {
            yield put({ type: FORGOT_PASSWORD_FORM_NAME_FAILURE, payload: { _error: data.data.msg } });
        } else {
            yield put({ type: FORGOT_PASSWORD_FORM_NAME_SUCCESS, payload: data });
        }
    }
        ...
        //const errors = Object.assign({}, error.errors, { _error: error.message || '' });
        const errors = Object.assign({}, error.errors, { _error: 'Xảy ra lỗi. Vui lòng kiểm tra lại!' });
        //-- The following dispatch will also call SubmissionError
        yield put({ type: FORGOT_PASSWORD_FORM_NAME_FAILURE, payload: errors });
    }
}
...
export function* doCreateNewPassword({ payload }) {
    try {
        ...
        if(data.status !== 1) {
            yield put({ type: PASSWORD_RESET_FORM_NAME_FAILURE, payload: {_error: data.msg} });
            return;
        }

        yield put({ type: PASSWORD_RESET_FORM_NAME_SUCCESS, payload: data.msg });
    }
    catch (error) {
        const errorMsg = error.response && error.response.data && error.response.data.message;
