...
class ThuHoiGCNListFilter extends Component {
    render() {
        const {userData, userGroups, handleSubmit, submitting, pristine, invalid} = this.props;
        
        let userGroupsFilter = userGroups.map(item => {return {label: item.group_name, value: item.id }});
        let listUserGroup = []; 

        if(userData.role_ids && userData.role_ids.indexOf(1) !== -1) {
            listUserGroup = [{label: "Tất cả", value: "all"}, ...userGroupsFilter];
        } else {
            listUserGroup = userGroupsFilter;
        }
...
export default ThuHoiGCNListFilter;