...
import {reduxForm, formValueSelector, getFormValues, change, reset} from "redux-form";
...
import {loadAHopDongInfo, clearAHopDong, uploadHopDongFiles, deleteAHopDongFile, requestTNMTLandData} from '../actions';
...
import {
    getContractTypeList, getLoaiHopDongList, getSoLuuTruList, getLoaiSoLuuTruList, getDoiTuongQuery, getDoiTuongQueryType, getTaiSanQuery,
    getTaiSanQueryIndex, getDoiTuongQueryIndex, getDistrictList, getWardList, getTaiSanHistories, getTaiSanInsertContract, getDoiTuongInsertContract
    ...
    componentDidUpdate(prevProps) {
        ...
        if(contractDetail && !prevProps.contractDetail || 
            ...
            let initialForm = {
                ...
                group_name: contractDetail && contractDetail.groupDetail && contractDetail.groupDetail.group_name 
                         || userData && userData.userGroupDetail.group_name,
                group_address: contractDetail && contractDetail.groupDetail && contractDetail.groupDetail.address 
                            || userData && userData.userGroupDetail.address,
        ...
        if(!isEmpty(assetQuery) && (assetQuery.id !== prevProps.assetQuery.id)) {
            ...
            if(findAsset) {
                ...
                //     let findOneAssetDetail = assetDetails.find(item => item.id === assetQuery.id);
                //     if(!findOneAssetDetail) {
                //         error = "Đang trong giao dịch khác";
                //     } else {
                //         findAsset.asset_status_id = 1;
                //     }
                // }
        ...
        if(findAsset) {
            this.props.openModalAction({
                ...
                onConfirm: () => {
                    ...
                    this.handleModalChange(100); // Modal Request TNMT = 100
                },
            });
        }
    }
