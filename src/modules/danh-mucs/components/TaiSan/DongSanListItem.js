...
const DongSanListItem = ({userData, asset, isEnableAddEdit, handleShowHistoryClick, handleDeleteClick}) => {
    const { 
        id, sub_asset_type_detail, owner_details, license_number, 
        issued_at, issued_by, asset_status_detail, created_at, created_by, createdByDetail,
    } = asset;
    ...
    if(asset_status_detail) {
        ...
        // } else if(asset_status_detail.id === 2){
        //     statusComponent = <span className="text-danger">{asset_status_detail && asset_status_detail.name}</span> 
        // } else {
...
DongSanListItem.propTypes = {
    asset: PropTypes.object.isRequired
};

export default DongSanListItem;