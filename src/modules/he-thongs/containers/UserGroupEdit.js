...
import {loadListUserGroup, loadAUserGroupInfo, clearAUserGroup} from '../actions';
import {getUserGroupList, getUserGroupInfo} from "../selectors";
...
class UserGroupEdit extends Component {
    componentDidMount() {
        ...
        if (id) {
            this.props.loadAUserGroupInfo({id});
        }
    ...
    componentWillUnmount() {
        this.props.clearAUserGroup();
    }

    componentDidUpdate(prevProps) {
        const {userGroupDetail} = this.props;
        if (userGroupDetail && userGroupDetail.id !== prevProps.userGroupDetail.id) {
            let initialForm = {
                ...userGroupDetail,
            }
...
const mapStateToProps = (state) => ({
    ...
    userGroupDetail:            getUserGroupInfo(state),
...
const mapDispatchToProps = (dispatch) => ({
    loadAUserGroupInfo:             payload => dispatch(loadAUserGroupInfo(payload)),
    loadListUserGroup:              payload => dispatch(loadListUserGroup(payload)),
    loadListDistrict:               payload => dispatch(loadListDistrict(payload)),
    clearAUserGroup:                payload => dispatch(clearAUserGroup(payload)),
...
const onSubmitSuccess = (result, dispatch) => {
    let returnData = {
        ...
        "status": true,
        "color": "success",
        "message": "Bạn đã sửa '" + result.data.data.group_name + "' thành công!"
    }
